...
a   = 1.0e-5;         % m (fixed phloem radius)
...
L_values = logspace(-2, 2, 5);       % m (e.g. 0.01 to 100)
k_values = logspace(-15, -12, 5);    % permeability (m^2)

% Storage: [k, L, c_max, J_max]
results = zeros(numel(k_values)*numel(L_values), 4);
row = 0;
...
ylabel('J(c)');
title('J vs c for 5×5 grid of (k, L) with a fixed');

colors = lines(numel(k_values)*numel(L_values)); % distinct colors
...
fprintf('All (k, L) combinations with their maxima:\n');
fprintf('---------------------------------------------------------------\n');
fprintf('   k            L (m)        c_max (mol/m^3)       J_max\n');
fprintf('---------------------------------------------------------------\n');
